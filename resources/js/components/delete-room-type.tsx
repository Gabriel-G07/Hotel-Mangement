...
export default function DeleteRoomType({ roomType }: DeleteRoomTypeProps) {
    const passwordInput = useRef<HTMLInputElement>(null);
    const { data, setData, delete: destroy, processing, reset, errors, clearErrors } = useForm<{ password: string }>({ password: '' });

    const deleteRoomType: FormEventHandler = (e) => {
        e.preventDefault();

        destroy(route('management.settings.room_types.destroy', roomType.room_type_id), {
            preserveScroll: true,
            onSuccess: () => reset(),
            onError: (error) => {
    ...
    return (
        <Dialog>
            <DialogTrigger asChild>
                <Button variant="destructive">Delete Room Type</Button>
            </DialogTrigger>
            <DialogContent>
                <DialogTitle>Are you sure you want to delete this room type?</DialogTitle>
                <DialogDescription>
                    Please enter your password for confirmation.
                </DialogDescription>
                <form onSubmit={deleteRoomType}>
