...
interface DeleteDutyProps {
    role: any;
}

export default function DeleteDuty({ role }: DeleteDutyProps) {
    const passwordInput = useRef<HTMLInputElement>(null);
    const { data, setData, delete: destroy, processing, reset, errors, clearErrors } = useForm<{ password: string }>({ password: '' });

    const deleteDuty: FormEventHandler = (e) => {
        e.preventDefault();

        destroy(route('roles.destroy', role.role_id), {
            preserveScroll: true,
            onSuccess: () => reset(),
            onError: (error) => {
    ...
    return (
        <Dialog>
            <DialogTrigger asChild>
                <Button variant="destructive">Delete Duty</Button>
            </DialogTrigger>
            <DialogContent>
